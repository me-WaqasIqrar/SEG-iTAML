    ...
    def train(self, model, epoch):
        ...
        if getattr(self.args, 'segmentation', False):
            ...
            for batch_idx, batch in enumerate(self.trainloader):
                ...
                # Convert targets to numpy for task identification
                np_targets = targets.detach().cpu().numpy()
                
                # Initial forward pass for logging (before meta-updates)
                with torch.no_grad():
                    logits_init, _ = model(inputs)
                
                # 2️⃣ Loop through all seen tasks (sessions 0 to current session)
                for task_idx in range(1 + self.args.sess):
                    # Task classes in targets (1-indexed): task_idx*class_per_task+1 to (task_idx+1)*class_per_task
                ...
                # Compute metrics after meta-update
                with torch.no_grad():
                    logits, _ = model(inputs)
                    preds = torch.argmax(logits, dim=1)
                    final_loss = F.cross_entropy(logits, seg_maps)
